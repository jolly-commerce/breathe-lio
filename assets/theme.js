...
// js/common/cart/shipping-cost-display.js - TEST-CART_001
var ShippingCostDisplay = class extends HTMLElement {
  static get observedAttributes() {
    return ["threshold", "total-price"];
  }
  constructor() {
  ...
  async connectedCallback() {
    ...
    // Also listen for when cart drawer opens to ensure we have latest state
  ...
  async attributeChangedCallback(attribute, oldValue, newValue) {
    if (attribute === "total-price") {
      this._updateDisplay();
    }
  }
  _updateDisplay() {
    const valueElement = this.querySelector(".text-subdued");
    if (valueElement) {
      const threshold = this.threshold;
      const totalPrice = this.totalPrice;

      // Debug logging
      console.log('ShippingCostDisplay _updateDisplay:', {
        threshold: threshold,
        totalPrice: totalPrice,
        isFreeShipping: totalPrice >= threshold
      });

      if (!isNaN(threshold) && !isNaN(totalPrice)) {
        if (totalPrice >= threshold) {
          valueElement.textContent = this.getAttribute("free-label");
        } else {
          valueElement.textContent = this.getAttribute("calculated-label");
        }
      }
    }
  }
  _onCartChanged(event) {
    ...
    // Debug logging
    console.log('ShippingCostDisplay cart changed:', {
      threshold: this.threshold,
      newTotalPrice: newTotalPrice,
      isFreeShipping: newTotalPrice >= this.threshold
    });

    this.totalPrice = newTotalPrice;
  }
};
if (!window.customElements.get("shipping-cost-display")) {
  window.customElements.define("shipping-cost-display", ShippingCostDisplay);
}
// TEST-CART_001 - END
